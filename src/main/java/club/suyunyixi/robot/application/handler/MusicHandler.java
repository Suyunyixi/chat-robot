...
@Slf4j
@Service
@MessageHandlerStrategy(handler = MessageHandler.MUSIC)
public class MusicHandler extends AbstractHandler {
    @Resource
    private UmongQuery query;

    @Override
    public BaseRespMessage explain(BaseParam param, BaseContext data) {
        data.setRespAts(ListUtil.toList(param.getAt()));
        UomgMusicDTO dto = query.music();
